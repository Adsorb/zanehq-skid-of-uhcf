...
public class TKeyCommand implements CommandExecutor, TabCompleter {

	private final Base plugin;

	public TKeyCommand(Base plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if (args.length == 0 || args.length > 2) {
            sender.sendMessage(
                    "§9§m--------------------------------");
            sender.sendMessage(
            		"§c/triplekey start <duration> - Starts the Triple Key timer.");
            sender.sendMessage(
            		"§c/triplekey end - Ends the current Triple Key timer.");
            sender.sendMessage(
                    "§9§m--------------------------------");
			return true;
		}
		...
		if (args.length == 1) {
			if (args[0].equalsIgnoreCase("end")) {
				if (plugin.getTimerManager().getTKeyTimer().clearCooldown()) {
					Bukkit.broadcastMessage(ChatColor.DARK_GREEN.toString() + ChatColor.STRIKETHROUGH
							+ BukkitUtils.STRAIGHT_LINE_DEFAULT);
					Bukkit.broadcastMessage(ChatColor.GREEN + "The Triple Key has been cancelled!");
					Bukkit.broadcastMessage(ChatColor.DARK_GREEN.toString() + ChatColor.STRIKETHROUGH
							+ BukkitUtils.STRAIGHT_LINE_DEFAULT);
					return true;
				}
		...
		if (args.length == 2) {
			if (args[0].equalsIgnoreCase("start")) {
				...
				if (duration < 1000L) {
					sender.sendMessage(ChatColor.RED + "Triple Key time must last for at least 20 ticks.");
					return true;
				}

				TKeyTimer tkeytimer = plugin.getTimerManager().getTKeyTimer();

				if (!tkeytimer.setRemaining(duration, true)) {
					sender.sendMessage(ChatColor.RED + "The Triple Key is already on.");
					return true;
				}
				...
				Bukkit.broadcastMessage(ChatColor.GRAY + "The TripleKey" + ChatColor.GRAY + " has started at " + ChatColor.DARK_GREEN + ConfigurationService.DONATE_URL
